...
export default function EditAddress() {
    const { address, coords, id } = JSON.parse(localStorage.getItem('@gestor/editAddress'));
    ...
    function handleSubmit() {
        if (reference.length > 1 && number.length > 1) {
            ...
            const listWithoutItem = listAddress.filter(item => item.id !== data.id)
            const newListAddress = [...listWithoutItem, data]
            localStorage.setItem('@gestor/listAddress', JSON.stringify(newListAddress))
            localStorage.removeItem('@gestor/editAddress')
